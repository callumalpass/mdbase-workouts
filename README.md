...
- **Templates**: A collapsible list of reusable workout blueprints (plan templates). Each template card displays its title, exercises with target sets/reps/weight, a "Plan" action that creates a dated plan from the template, and a "Start" button that opens the session logger directly.
...
- **plan** (`plans/`): A scheduled workout for a specific date. Fields include `date` (the date the workout is planned for, not when the file was created), `title`, `status` (`scheduled`, `completed`, or `skipped`), an ordered list of `exercises` with optional `target_sets`, `target_reps` (number-like strings, ranges, or labels such as `AMRAP`), and `target_weight`, and a `session` link that points to the session record created when the plan is executed. Plans appear on the Today tab if their date matches today, and on the Calendar tab for their respective date.

- **plan-template** (`plan-templates/`): A reusable workout blueprint without a date. It uses the same exercise target structure as a plan, including flexible `target_reps` values like "8", "10-12", or "AMRAP". Templates appear in a collapsible section on the Today tab and can either be scheduled as dated plans or started directly into the session logger. Sessions logged directly from templates are standalone (no plan link).
...
- `npm test` - run Vitest
- `npm run test:manifest` - validate the Connect manifest and install its type pack into a disposable collection
